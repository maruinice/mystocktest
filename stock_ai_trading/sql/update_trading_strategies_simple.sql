...
-- 2. 修改现有字段
ALTER TABLE trading_strategies MODIFY COLUMN strategy_type ENUM('technical', 'fundamental', 'quantitative', 'ai_driven', 'trend_following', 'mean_reversion', 'momentum', 'arbitrage', 'multi_factor', 'volatility', 'custom') NOT NULL COMMENT '策略类型';
ALTER TABLE trading_strategies MODIFY COLUMN status ENUM('draft', 'active', 'inactive', 'testing', 'paused', 'stopped', 'error') DEFAULT 'draft' COMMENT '策略状态';
